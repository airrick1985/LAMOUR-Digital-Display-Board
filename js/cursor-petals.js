/* js/cursor-petals.js — 全域滑鼠拖尾特效：小花瓣 + 蝴蝶（白/淡粉/金） */
(() => {
    ...
    class TrailButterfly {
        constructor(x, y, dx, dy) {
            ...
            this.size = Math.random() * 4 + 8;   // 8~12px
            this.life = 1;
        ...
        drawWing(s, flap) {
            ...
            ctx.beginPath(); // 上翼
            ctx.moveTo(0, 0);
            ctx.bezierCurveTo(s * 1.1, -s * 1.0, s * 1.3, -s * 0.2, 0.5, s * 0.05);
            ctx.fill();
            ctx.beginPath(); // 下翼
            ctx.moveTo(0, s * 0.1);
            ctx.bezierCurveTo(s * 0.9, s * 0.5, s * 0.5, s * 1.0, 0.5, s * 0.2);
            ctx.fill();
            ctx.restore();
        }
        ...
        draw(t) {
            const s = this.size;
            // 拍翅：兩翼在 0.15~1 之間開合
            const flap = 0.15 + Math.abs(Math.sin(t * this.flapSpeed + this.flapPhase)) * 0.85;
            const [r, g, b] = this.color;
            ...
            // 身體：細長橢圓，用深灰藍與翅膀做出區隔
            ctx.fillStyle = 'rgba(74, 87, 89, 0.85)';
            ctx.beginPath();
